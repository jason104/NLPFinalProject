...
def main():
  ...
  predicted = [testList[i] if prediction[i][j] == 1 else '' for i in range(len(prediction)) for j in range(18)]
  print(predicted, ids, len(predicted), len(ids))
  outDataFrame = pd.DataFrame({'id-#aspect': ids, 'predicted': predicted})
  outDataFrame.to_csv('task1.csv', index=False)
